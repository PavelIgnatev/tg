        ...
        if (managerMessage && managerMessage.trim()) {
          ...
          const message = await makeRequestGPT(
            [
              {
                role: "system",
                content: `Вам будет предложено предложение на неизвестном языке, вам нужно определить язык, после перевести предложение на ${offer.language} язык. В ответе вернуть только предложение, переведенное на ${offer.language} язык!`,
              },
              {
                role: "user",
