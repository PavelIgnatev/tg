        ...
        if (managerMessage && managerMessage.trim()) {
          ...
          const message = await makeRequestGPT(
            [
              {
                role: "system",
                content: `Ты выполняешь роль функции, переводящей переданное сообщение на ${offer.language} язык. Увеличивать или уменьшать длину сообщения запрещено, необходимо только перевести сообщение. В ответе вернуть только результат - переведенное сообщение, основной язык в котором - ${offer.language}, без дополнительных префиксов. Только ${offer.language} язык.`,
              },
              {
                role: "user",
