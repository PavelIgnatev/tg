    ...
    while (!isSender) {
      ...
      try {
        ...
        if (managerMessage && managerMessage.trim()) {
          ...
          Верни только одно ответное сообщение к ${userNameFilter}, в котором будет содержаться основаная часть-отсылка на последнее сообщение с наводящим вопросом для продолжения общения.
          # ${botName}:`,
              true
            )
          )
