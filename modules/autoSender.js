...
const autoSender = async (accountId, context, account) => {
  ...
  try {
    ...
    const translatePrompt = `Ты выполняешь роль функции, переводящей переданное сообщение на ${realLanguage} язык. Увеличивать или уменьшать длину сообщения запрещено, необходимо только перевести сообщение. В ответе вернуть только результат - переведенное сообщение, основной язык в котором - ${realLanguage}, без дополнительных префиксов. Только ${realLanguage} язык.`;
