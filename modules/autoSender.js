...
const autoSender = async (accountId, context, account) => {
  ...
  try {
    ...
    const translatePrompt = `Вам будет предложено предложение на неизвестном языке, вам нужно определить язык, после перевести предложение на ${realLanguage} язык. В ответе вернуть только предложение, переведенное на ${realLanguage} язык!`;
