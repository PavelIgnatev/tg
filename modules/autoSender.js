...
const defaultSecondMessageWithBioPrompt =
  "Примите роль составителя вопросов. Вам нужно создать вопрос, который будет кратким (до 100 символов, но не менее 30 символов), содержательным и связанным с описанием деятельности пользователя. Описание пользователя: ${userDescription}";
...
const autoSender = async (accountId, context, account) => {
  ...
  try {
    ...
    const firstMessagePrompt = `Тебе предстоит выполнить задачу анализа содержимого переменной ${userData}. Твоя цель - выявить в этой переменной имя пользователя. Как только ты определишь имя, используй его для создания персонализированного приветствия. Приветствие должно быть на русском языке и начинаться со слов "Здравствуйте". Если в предоставленных данных имя обнаружить не удастся, остановись на формулировке "Здравствуйте" без добавления имени. В случае обнаружения имени пользователя, включи его в приветствие. Если имя представлено не на русском языке, проведи его транслитерацию или перевод так, чтобы оно звучало естественно и корректно в контексте русского приветствия, например: "Здравствуйте, Джон!". Удели особое внимание естественности обращения и помни, что все ответы должны быть предоставлены исключительно на русском языке.`;
